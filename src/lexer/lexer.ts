  ...
  */
  ...
  private ignoreComments(
    ...
    function skipLineComment() {
      while (!isEmpty(source.get(position)) && source.get(position) != "\n") {
        position++;
      }
    }

    function skipInlineComment() {
      while (!isEmpty(source.get(position)) && source.get(position) != "*" && source.get(position + 1) != "/") {
        position++;
      }
    }
  }
...
function parseNumberToken(
  ...
  function getDigits() {
    let digits = "";
    while (!isEmpty(source.get(position))) {
...
function parseIdentifierToken(
  ...
  while (!isEmpty(source.get(position)) && !spaceCharacters.has(source.get(position))) {
...
function parseAssignmentToken(
  ...
  return [{type: TokenType.Assignment, end: position}];
}
...
function parseEOFToken(
  ...
  if (!isEmpty(source.get(initialPosition))) {
    return null;
  }
