import { TokenChoice } from "../parser/branches/parserState.ts";
...
export type TokenValue =
  ...
  & (
    ...
    // To do later
    | { type: TokenType._lifetime } & never
    | { type: TokenType._variableType } & never
  );
...
export function getToken(startPosition: number, tokens: Token[]): Token | undefined {
  ...
  if (!hasTokensLeft(startPosition, tokens)) {
...
export function getTokenValues(token: Token | undefined, types: TokenType[]): TokenValue[] {
  if (token === undefined || types.length === 0) {
    return [];
  }
